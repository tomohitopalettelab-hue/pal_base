...
export async function POST(req: Request) {
  try {
    ...
    if (!settings?.gbpRefreshToken) return NextResponse.json({ success: false, error: 'リフレッシュトークンがありません。再度OAuth連携してください。' }, { status: 400 });

    const clientId = process.env.GOOGLE_CLIENT_ID;
    const clientSecret = process.env.GOOGLE_CLIENT_SECRET;
    if (!clientId || !clientSecret) return NextResponse.json({ success: false, error: 'Google OAuth設定が不足しています。' }, { status: 500 });
    ...
    if (!data.access_token) throw new Error(data.error_description || 'トークン更新に失敗しました。');

    await upsertSettings(paletteId, { gbpAccessToken: data.access_token });
    return NextResponse.json({ success: true });
  } catch (err: unknown) {
    return NextResponse.json({ success: false, error: err instanceof Error ? err.message : 'トークン更新に失敗しました。' }, { status: 500 });
  }
}
