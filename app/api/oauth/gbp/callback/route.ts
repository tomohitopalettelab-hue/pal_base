...
export async function GET(req: Request) {
  ...
  try {
    ...
    if (!tokenData.access_token) throw new Error(tokenData.error_description || 'アクセストークンの取得に失敗しました。');

    const accessToken: string = tokenData.access_token;
    const refreshToken: string = tokenData.refresh_token || '';

    const accountsRes = await fetch('https://mybusinessaccountmanagement.googleapis.com/v1/accounts', { headers: { Authorization: `Bearer ${accessToken}` } });
    const accountsData = await accountsRes.json();
    const accounts: { name: string }[] = accountsData.accounts || [];

    if (accounts.length === 0) throw new Error('GBPのアカウントが見つかりません。GBPに登録されているGoogleアカウントでログインしてください。');

    const locations: { id: string; name: string; address: string }[] = [];
    for (const account of accounts) {
      const locRes = await fetch(`https://mybusinessbusinessinformation.googleapis.com/v1/${account.name}/locations?readMask=name,title,storefrontAddress`, { headers: { Authorization: `Bearer ${accessToken}` } });
      const locData = await locRes.json();
      for (const loc of (locData.locations || [])) {
        const addr = loc.storefrontAddress ? [loc.storefrontAddress.locality, loc.storefrontAddress.administrativeArea].filter(Boolean).join(', ') : '';
        locations.push({ id: loc.name, name: loc.title || loc.name, address: addr });
      }
    }

    if (locations.length === 0) throw new Error('ロケーションが見つかりません。GBPに店舗情報を登録してください。');

    if (locations.length === 1) {
      await upsertSettings(paletteId, { gbpAccessToken: accessToken, gbpRefreshToken: refreshToken, gbpLocationId: locations[0].id });
      return NextResponse.redirect(`${redirectBase}?gbp_connected=1&paletteId=${encodeURIComponent(paletteId)}&gbp_name=${encodeURIComponent(locations[0].name)}`);
    }
    ...
    return NextResponse.redirect(`${redirectBase}?gbp_select=1&paletteId=${encodeURIComponent(paletteId)}&locations=${encoded}`);
  } catch (err: unknown) {
    const msg = encodeURIComponent(err instanceof Error ? err.message : 'GBP連携に失敗しました。');
