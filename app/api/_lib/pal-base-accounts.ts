...
export const listPalBaseAccountsFromPalDb = async (): Promise<PalDbAccount[]> => {
  ...
  const targetPlanIds = new Set(
    plans
      .filter((plan) => isPalBasePlanCode(plan.code))
  ...
  const standardPlanIds = new Set(
    plans
      .filter((plan) => isPalBaseStandardPlanCode(plan.code))
      .map((plan) => String(plan.id || '').trim())
      .filter(Boolean),
  );
...
export const findPalBaseAccountByPaletteId = async (paletteId: string): Promise<PalDbAccount | null> => {
...
export const canLoginPalBaseByPaletteId = async (paletteId: string): Promise<boolean> => {
  const target = String(paletteId || '').trim().toUpperCase();
